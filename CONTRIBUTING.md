...
- **Backward compatibility is a hard gate — ask before breaking it.** This library is vendored into other applications, so a change that could break existing consumers must not be made unilaterally: open an issue (or say so in your PR) describing the break and who it affects, and get a maintainer's agreement before proceeding. This covers renaming/removing/changing the signature of any public method, static config array, option key, or property; changing a return type, thrown-exception type, or observable behavior/defaults; altering a connection-string or `Config` format; tightening validation; and raising the PHP floor or a dependency constraint. When unsure whether a change is breaking, treat it as breaking and ask.
