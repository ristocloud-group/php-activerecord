...
- Fix bugs.
- Upgrade the codebase to run on **PHP 8.5** (composer.json currently declares `^7.4|^8.0`; CI matrix currently tops out at 8.2).
...
Everything runs inside Docker (the `tests` service has PHP + all PDO extensions + memcached). The MySQL/Postgres/memcached containers must be up because most tests hit a real database.

```sh
docker compose up -d                                    # boot mysql, postgres, memcached, tests
...
docker compose exec tests composer run check-compatibility             # PHPCompatibility static analysis
```

`composer run test` = `phpunit --fail-on-risky --fail-on-warning --fail-on-skipped --testdox`. **Skipped tests fail the build** — a test that skips because a dependency (e.g. memcached, a DB) is unavailable is a red build, not a pass. Design work so nothing skips in the Docker environment.

Coverage is not wired into `phpunit.xml` yet. To measure it (xdebug/pcov must be available in the image):
```sh
...
```sh
docker compose build --build-arg PHP_VERSION=8.3 && docker compose up -d
```
CI (`.circleci/config.yml`) runs this matrix per push: PHP 7.4, 8.0, 8.1, 8.2. When bumping to 8.5, update the CI matrix, `composer.json` `require.php`, the `check-compatibility` `testVersion` range, and `docker-compose.yml`'s default `PHP_VERSION`.
...
- The codebase predates modern PHP style: `array()` literals, few type declarations, tabs for indentation. Match the surrounding file rather than modernizing wholesale — but the PHP 8.5 upgrade will require targeted fixes (dynamic properties, `#[\ReturnTypeWillChange]`, deprecated `each()`/curly-brace access, nullable-parameter deprecations, etc.). When touching a hot path, prefer minimal, behavior-preserving edits.

## Tests

`phpunit.xml` bootstraps `test/helpers/config.php`, which registers connections from env vars (`PHPAR_MYSQL`, `PHPAR_PGSQL`, `PHPAR_SQLITE`, `PHPAR_MEMCACHED` — set in `docker-compose.yml`) and sets `mysql` as the default connection.
...
- **Adapter-specific tests** (`MysqlAdapterTest`, `SqliteAdapterTest`, ...) extend a shared `AdapterTest` and run the same battery against each backend by overriding the connection name.
